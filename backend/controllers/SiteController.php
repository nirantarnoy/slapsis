...
//    }
    ...
    public function actionTiktokCallback()
    {
        ...
        Yii::info("TikTok Callback received at: " . date('Y-m-d H:i:s'), __METHOD__);
        Yii::info("Callback URL: {$fullUrl}", __METHOD__);
        ...
        // เริ่ม token exchange ทันที (ภายใน 10 วินาที)
        $appKey = '6h9n461r774e1';
        $appSecret = '1c45a0c25224293abd7de681049f90de3363389a';

        $client = new \GuzzleHttp\Client(['timeout' => 10]); // ลด timeout

        // ลำดับความสำคัญของ endpoints และ parameters
        $attempts = [
            [
        ...
        foreach ($attempts as $attemptIndex => $attempt) {
            ...
            try {
                Yii::info("Attempt #{$attemptIndex}: {$attempt['endpoint']}", __METHOD__);
                ...
                Yii::info("Attempt #{$attemptIndex} completed in {$attemptTime}ms, status: {$statusCode}", __METHOD__);
                Yii::info("Response: {$raw}", __METHOD__);

                if ($statusCode === 200) {
                    $data = json_decode($raw, true);

                    if (json_last_error() === JSON_ERROR_NONE &&
                        (isset($data['data']['access_token']) || isset($data['access_token']))) {

                        // SUCCESS - ได้ token แล้ว
                        $totalTime = round((microtime(true) - $startTime) * 1000, 2);
                        Yii::info("SUCCESS in {$totalTime}ms with attempt #{$attemptIndex}", __METHOD__);
                        ...
                        if ($this->saveTikTokToken($saveId, $tokenData)) {
                            if ($finalShopId) {
                                Yii::$app->session->setFlash('success', "เชื่อมต่อ TikTok สำเร็จ! Shop ID: {$finalShopId} (ใช้เวลา {$totalTime}ms)");
                            } else {
                                Yii::$app->session->setFlash('warning', "เชื่อมต่อสำเร็จ แต่ไม่พบ Shop ID (Temp ID: {$saveId})");

                                // ลองหา shop ID ภายหลัง
                                $this->tryGetShopId($tokenData['access_token'], $saveId);
                            }
                        } else {
        ...
        Yii::$app->session->setFlash('error',
            ...
            'อาจเป็นเพราะ code หมดอายุ (ใช้เวลา ' . $totalTime . 'ms)<br>' .
